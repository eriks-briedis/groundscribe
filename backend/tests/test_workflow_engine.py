...
@pytest.fixture
def engine(recorder: ProvenanceRecorder, snapshot_store: SnapshotStore) -> WorkflowEngine:
...
def snapshot(
    ...
    *,
    ...
    return snapshots.write(
        ...
        content=content,
        created_by_execution_id=None if orphan else engine.execution.id,
        parent=parent,
    )
...
def test_every_transition_emits_a_decision_naming_its_policy(engine: WorkflowEngine) -> None:
    """plan/05: every transition emits a decision record naming its policy/actor."""
    recorded = engine.apply(A.EXTRACT_SOURCE_MODEL)
...
def test_a_transition_carries_the_edge_rationale_into_the_record(
    ...
    engine = build_engine(recorder, snapshot_store, state=S.SUBSTANTIVE_REVIEWING)
    assert engine.apply(A.ACCEPT_REVIEW).decision.rationale


def test_a_user_transition_is_attributed_to_the_person(
    recorder: ProvenanceRecorder, snapshot_store: SnapshotStore
) -> None:
    engine = build_engine(recorder, snapshot_store, state=S.ARCHITECTURE_REVIEW_REQUIRED)
...
def test_a_user_transition_without_a_name_is_refused(
    ...
    with pytest.raises(AttributionRequired):
        engine.apply(A.APPROVE_ARCHITECTURE, actor_type=ActorType.USER)
    assert engine.state is S.ARCHITECTURE_REVIEW_REQUIRED


def test_transitions_appear_on_the_run_timeline(
    engine: WorkflowEngine, db_session: Session
) -> None:
    engine.apply(A.EXTRACT_SOURCE_MODEL)
    engine.apply(A.COMPLETE_EXTRACTION)
    moves = [
        event
        for event in timeline(db_session, engine.run.correlation_id)
...
def test_the_engine_offers_the_actions_of_its_current_state(engine: WorkflowEngine) -> None:
    assert engine.available_actions() == (A.CANCEL, A.EXTRACT_SOURCE_MODEL, A.FAIL)
...
def test_a_routing_decision_identifies_the_policy_that_made_it(
    ...
    engine = build_engine(recorder, snapshot_store, state=S.REVISION_REQUIRED)
    decision = engine.route(C.STYLE_ISSUE).decision
...
def test_an_approved_extra_round_is_attributed_to_the_approver(
    ...
    engine = build_engine(recorder, snapshot_store, state=S.REVISION_REQUIRED)
    for _ in range(3):
        engine.route(C.SUBSTANTIVE_ISSUE)
        fail_again(engine.machine)

    recorded = engine.route(
...
def test_an_exhausted_limit_records_the_stall_and_asks_for_a_person(
    ...
    recorded = engine.route(C.SUBSTANTIVE_ISSUE)
...
def test_a_stagnation_stall_records_the_findings_that_caused_it(
    ...
    recorded = engine.check_stagnation(history)
    ...
    assert engine.state is S.STALLED
...
def test_a_healthy_check_records_nothing_and_moves_nothing(
    ...
    assert engine.state is S.REVISION_REQUIRED
...
def test_an_artefact_without_a_creating_execution_blocks_the_transition(
    engine: WorkflowEngine, snapshot_store: SnapshotStore
) -> None:
    """plan/05: every generated artefact references a creating execution."""
    orphan = snapshot(engine, snapshot_store, ArtifactType.SOURCE_MODEL, orphan=True)
    with pytest.raises(ArtifactProvenanceError):
        engine.apply(A.EXTRACT_SOURCE_MODEL, artifacts=(orphan,))
    assert engine.state is S.SOURCE_INGESTED


def test_a_blocked_transition_records_no_decision(
    engine: WorkflowEngine, snapshot_store: SnapshotStore, db_session: Session
) -> None:
    """A guard that fired after the write would leave a decision for a move
    that never happened."""
    orphan = snapshot(engine, snapshot_store, ArtifactType.SOURCE_MODEL, orphan=True)
    before = len(timeline(db_session, engine.run.correlation_id))
    with pytest.raises(ArtifactProvenanceError):
        engine.apply(A.EXTRACT_SOURCE_MODEL, artifacts=(orphan,))
    assert len(timeline(db_session, engine.run.correlation_id)) == before
...
def test_an_attributed_artefact_is_linked_to_the_transition(
    ...
    produced = snapshot(engine, snapshot_store, ArtifactType.SOURCE_MODEL)
    recorded = engine.apply(A.EXTRACT_SOURCE_MODEL, artifacts=(produced,))
    assert produced.id in recorded.decision.inputs["artifacts"]
    assert produced.id in {artifact.snapshot_id for artifact in engine.execution.artifacts}
...
def approve_architecture(engine: WorkflowEngine, snapshots: SnapshotStore) -> ArtifactSnapshot:
    approved = snapshot(engine, snapshots, ArtifactType.CONTENT_ARCHITECTURE, b'{"v":1}')
    engine.apply(
...
def test_an_approved_architecture_cannot_be_replaced_silently(
    ...
    engine = build_engine(recorder, snapshot_store, state=S.ARCHITECTURE_REVIEW_REQUIRED)
    approve_architecture(engine, snapshot_store)

    unrelated = snapshot(engine, snapshot_store, ArtifactType.CONTENT_ARCHITECTURE, b'{"v":2}')
    with pytest.raises(SilentMutationError):
        engine.apply(A.GENERATE_BRIEF, artifacts=(unrelated,))
    assert engine.state is S.ARCHITECTURE_APPROVED
...
def test_a_forked_architecture_still_needs_an_override_record(
    ...
    engine = build_engine(recorder, snapshot_store, state=S.ARCHITECTURE_REVIEW_REQUIRED)
    approved = approve_architecture(engine, snapshot_store)
    revised = snapshot(
        engine, snapshot_store, ArtifactType.CONTENT_ARCHITECTURE, b'{"v":2}', parent=approved
    )
    with pytest.raises(SilentMutationError):
        engine.apply(A.GENERATE_BRIEF, artifacts=(revised,))
...
def test_a_forked_architecture_with_an_override_is_allowed(
    ...
    assert engine.approved_architecture is not None
    assert engine.approved_architecture.id == revised.id
...
def test_an_override_cannot_launder_an_unrelated_architecture(
    ...
    engine = build_engine(recorder, snapshot_store, state=S.ARCHITECTURE_REVIEW_REQUIRED)
    approve_architecture(engine, snapshot_store)
    unrelated = snapshot(engine, snapshot_store, ArtifactType.CONTENT_ARCHITECTURE, b'{"v":9}')
    with pytest.raises(SilentMutationError):
        engine.apply(
            A.GENERATE_BRIEF,
            artifacts=(unrelated,),
...
def test_a_successor_article_version_must_retain_its_lineage(
    ...
    engine = build_engine(recorder, snapshot_store, state=S.DRAFT_GENERATING)
    first = snapshot(engine, snapshot_store, ArtifactType.ARTICLE_VERSION, b'{"draft":1}')
    engine.apply(A.SUBMIT_DRAFT, artifacts=(first,))
    engine.apply(A.REQUIRE_REVISION_PLAN)
    engine.apply(A.APPROVE_REVISION_PLAN, actor_id="ada", actor_type=ActorType.USER)

    orphaned = snapshot(engine, snapshot_store, ArtifactType.ARTICLE_VERSION, b'{"draft":2}')
    with pytest.raises(LineageError):
        engine.apply(A.SUBMIT_REWRITE, artifacts=(orphaned,))

    forked = snapshot(
        engine, snapshot_store, ArtifactType.ARTICLE_VERSION, b'{"draft":2}', parent=first
    )
    assert engine.apply(A.SUBMIT_REWRITE, artifacts=(forked,)).state is S.SUBSTANTIVE_REVIEWING
...
def test_export_must_use_the_version_that_passed_validation(
    ...
    engine = build_engine(recorder, snapshot_store, state=S.FINAL_VALIDATING)
    passed = snapshot(engine, snapshot_store, ArtifactType.ARTICLE_VERSION, b'{"final":1}')
    engine.apply(A.VALIDATION_PASSED, artifacts=(passed,))

    other = snapshot(
        engine, snapshot_store, ArtifactType.ARTICLE_VERSION, b'{"final":2}', parent=passed
    )
    with pytest.raises(ExportMismatchError):
        engine.apply(A.APPROVE_FINAL, actor_id="ada", actor_type=ActorType.USER, artifacts=(other,))
    assert engine.state is S.HUMAN_APPROVAL_REQUIRED
...
def test_approving_an_export_with_no_validated_version_is_refused(
    ...
    engine = build_engine(recorder, snapshot_store, state=S.HUMAN_APPROVAL_REQUIRED)
    version = snapshot(engine, snapshot_store, ArtifactType.ARTICLE_VERSION, b'{"final":1}')
    with pytest.raises(ExportMismatchError):
        engine.apply(
            A.APPROVE_FINAL, actor_id="ada", actor_type=ActorType.USER, artifacts=(version,)
        )
...
def test_confidential_material_cannot_reach_the_published_output(
    ...
    leaking = snapshot(
        engine, snapshot_store, ArtifactType.ARTICLE_VERSION, b'{"body":"as Project Zephyr showed"}'
    )
    engine.apply(A.VALIDATION_PASSED, artifacts=(leaking,))
    with pytest.raises(ConfidentialMaterialError, match="Project Zephyr"):
        engine.apply(
            A.APPROVE_FINAL, actor_id="ada", actor_type=ActorType.USER, artifacts=(leaking,)
        )
    assert engine.state is S.HUMAN_APPROVAL_REQUIRED
...
def test_clean_output_passes_the_confidentiality_guard(
    ...
    clean = snapshot(
        engine, snapshot_store, ArtifactType.ARTICLE_VERSION, b'{"body":"a caching write-up"}'
    )
    engine.apply(A.VALIDATION_PASSED, artifacts=(clean,))
    assert (
        engine.apply(
...
def test_a_replay_creates_a_new_execution_linked_to_the_original(
    engine: WorkflowEngine, recorder: ProvenanceRecorder
) -> None:
    """plan/05: replays cannot overwrite original executions."""
    original = engine.begin_stage("extract_claims")
    recorder.complete_stage(original)

    replay = engine.replay(original, requested_by="ada")
...
def test_a_replay_is_recorded_as_the_requester_s_decision(
    engine: WorkflowEngine, recorder: ProvenanceRecorder
) -> None:
    original = engine.begin_stage("extract_claims")
    recorder.complete_stage(original)
    engine.replay(original, requested_by="ada")
    decisions = [
        record
        for record in engine.execution.decisions
        if record.decision_type == "execution_replay"
    ]
    assert [record.decided_by for record in decisions] == ["ada"]


def test_a_failed_execution_keeps_its_trace(engine: WorkflowEngine, db_session: Session) -> None:
    """plan/05: failed executions retain their trace."""
    stage = engine.begin_stage("draft_article")
    engine.fail(error_type="provider_error", error_message="the model timed out", execution=stage)

    assert stage.status is ExecutionStatus.FAILED
    assert engine.state is S.FAILED
    events = [
        event
        for event in timeline(db_session, engine.run.correlation_id)
...
def test_a_stage_execution_belongs_to_the_run(engine: WorkflowEngine) -> None:
    stage = engine.begin_stage("extract_claims")
    assert stage.pipeline_run_id == engine.run.id
    assert stage.correlation_id == engine.run.correlation_id
