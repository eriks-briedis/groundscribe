              ...
              <ul className="cards">
                {index.projects.map((project) => (
                  <li key={project.id} className="card" data-testid={`project-${project.id}`}>
                    ...
                    <p className="muted">
                      {readable(project.state)} · {project.articles}{' '}
                      {project.articles === 1 ? 'article' : 'articles'} · opened {project.opened_at}
                    </p>
...
function NewProject({ actor, onCreated }: { actor: string; onCreated: () => void }) {
  ...
  return (
    <form className="panel new-project" onSubmit={(event) => void start(event)}>
